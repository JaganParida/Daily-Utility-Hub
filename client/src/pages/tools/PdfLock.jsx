...
import { Lock, UploadCloud, FileText, CheckCircle2 } from 'lucide-react';
...
const PdfLock = () => {
  ...
  const handleLock = async () => {
    ...
    try {
      ...
      const toastId = toast.loading('Encrypting PDF securely on server...');
      ...
      link.href = url;
      link.setAttribute('download', 'locked_document.pdf');
      ...
      if (error.response?.data instanceof Blob) {
         ...
         reader.onload = () => {
           try {
             ...
             toast.error(json.message || 'Failed to lock PDF', { id: toastId });
           } catch {
             toast.error('Failed to lock PDF', { id: toastId });
           }
         };
  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col min-h-[calc(100vh-140px)]">
      <div className="mb-6 flex items-center gap-3 shrink-0">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Lock PDF</h1>
          <p className="text-muted-foreground mt-1 text-sm">Encrypt your PDF with a password and restrict permissions.</p>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-6 overflow-hidden">
          ...
          {file && (
             <div className="bg-card border border-border p-6 rounded-2xl shadow-sm space-y-4">
               <label className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Secure Password</label>
               <input
                 type="password"
                 value={password}
                 onChange={(e) => setPassword(e.target.value)}
                 placeholder="Enter a strong password"
                 className="w-full bg-background border border-border rounded-xl px-4 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
               />
               <p className="text-xs text-muted-foreground">
                 This password will be required to open the document. Do not forget it!
               </p>
             </div>
          )}
          ...
          <div>
            ...
            <div className="space-y-4 text-sm text-foreground">
              <div className="flex items-start gap-3">
                ...
                <p>Uses strong AES-256 military-grade encryption.</p>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="text-emerald-500 mt-0.5 shrink-0" size={16} />
                <p>Disables text copying, modifying, and printing.</p>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="text-emerald-500 mt-0.5 shrink-0" size={16} />
                <p>Original file is completely deleted after processing.</p>
              </div>
            </div>
          </div>
